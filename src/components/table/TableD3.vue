    ...
    <table class="mainTable">
      <thead id="head" ref="thead">
        ...
        <tr id="mainTr">
          ...
          <th
            v-for="(col, i) in colsToRender.slice(0, 2)"
            :key="i"
            :class="getThClasses(col, i)"
          >
            <button
              v-if="i == 0"
              class="btn btn-link showMsaBtn"
              @click="toggleTabs"
            >
              <span class="text-danger">{{
          ...
          <th
            v-for="(col, i) in colsToRender.slice(2, 2 + n_annotations)"
            :key="i + 2"
            :class="getThClasses(col, i + 2)"
            rowspan="2"
          >
            <div
              v-if="!msaTab && n_annotations > 0 && i == 0"
              class="annoPopver"
            >
          ...
          <th
            v-for="(col, i) in colsToRender.slice(
              2 + n_annotations,
              5 + 2 + n_annotations
            )"
            :key="i + 2 + n_annotations"
            :class="getThClasses(col, i + 2 + n_annotations)"
          >
            <tablecell
              :content="getHeader(col, i + 2 + n_annotations)"
            ></tablecell>
            <b-button
    ...
    setTableRow(n, index) {
      ...
      if (n.data.uniprotId) {
        ...
        if (
          ...
          this.store_annoMapping.annoMap[uniprotId]
        ) {
