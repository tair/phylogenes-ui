...
const actions = {
  ...
  [types.TREE_ACTION_DO_SEARCH]: (context, payload) => {
    ...
    const {
      searchText,
      filters: { startRow, rows, species, organisms },
    } = context.state.tree.payload
    ...
    axios({
      ...
      .then((res) => {
        ...
        if (res.data.facet_counts.facet_fields.node_types != null) {
          for (
            var i = 0;
            i < res.data.facet_counts.facet_fields.node_types.length;

          ) {
            var jsonData = {}
            jsonData['key'] = res.data.facet_counts.facet_fields.node_types[i++]
            jsonData['count'] =
              res.data.facet_counts.facet_fields.node_types[i++]
        ...
        if (res.data.facet_counts.facet_fields.organisms != null) {
          for (
            var i = 0;
            i < res.data.facet_counts.facet_fields.organisms.length;

          ) {
            var jsonData = {}
            jsonData['key'] = res.data.facet_counts.facet_fields.organisms[i++]
            jsonData['count'] =
              res.data.facet_counts.facet_fields.organisms[i++]
